...
上一节中演示了基本的select \* from goods\_cates查询，可以获取到goods\_\_cates里的所有数据。但是当数据量大时，就需要各种合适的查询方式来获取指定的信息并呈现给我们。本节就来讲述并演示这些操作。在开始之间我们先介绍几个命令。
...
接下来，我们获取name为网卡的列，但不显示id。

```
mysql> select name from goods_cates where name="网卡";
+--------+
| name   |
+--------+
| 网卡   |
